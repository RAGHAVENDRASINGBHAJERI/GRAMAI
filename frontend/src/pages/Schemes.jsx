...
const Schemes = () => {
  ...
  const { content, locationName, isLoading, error, updateLocation } = useLocationContent('schemes');
      ...
      >
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-[300px] text-text-secondary">
            ...
            <p>Scanning active government schemes for your region...</p>
          </div>
        ) : error ? (
          ...
          <div className="w-full">
            <MarkdownRenderer content={content} colorTheme="blue" />
          </div>
        )}
      </motion.div>
    </div>
