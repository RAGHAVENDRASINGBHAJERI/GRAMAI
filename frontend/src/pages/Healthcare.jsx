...
const Healthcare = () => {
  ...
  const { content, locationName, isLoading, error } = useLocationContent('health');
      ...
      >
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-[300px] text-text-secondary">
            ...
            <p>Compiling local healthcare advisories...</p>
          </div>
        ) : error ? (
          ...
          <div className="w-full">
            <MarkdownRenderer content={content} colorTheme="red" />
          </div>
        )}
      </motion.div>
    </div>
